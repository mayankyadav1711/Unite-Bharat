...
const ViewProfile = () => {
  ...
  useEffect(() => {
    // Fetch user details and projects when component mounts
    fetchUserData();
  }, [id]);
  ...
  const fetchUserData = async () => {
    try {
      // Fetch user details
      const userDetailsResponse = await fetch(`https://api-sankalp.vercel.app/user-details/${id}`);
      const userDetailsData = await userDetailsResponse.json();
      setUserDetails(userDetailsData);

      // Fetch user projects
      const userProjectsResponse = await fetch(`https://api-sankalp.vercel.app/user-projects/${id}`);
...
<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap:"100px" }}>
 ...
 <h3 style={{ fontSize: '28px', textAlign: 'center', fontWeight: 'bold', margin: '20px 0' }}>Projects</h3>
